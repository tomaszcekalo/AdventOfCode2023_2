...
var input = @"Game 1: 12 red, 2 green, 5 blue; 9 red, 6 green, 4 blue; 10 red, 2 green, 5 blue; 8 blue, 9 red
...
Dictionary<string, int> cubeMax=new Dictionary<string, int>()
{
    {"red", 12},
    {"green", 13},
    {"blue", 14}
...
var sum = 0;
foreach(var line in input.Split(System.Environment.NewLine))
{
    var split= line.Split(":");
    var gameID = split[0].Replace("Game ", "").Trim();
    ...
    foreach(var set in sets)
    {
        ...
        if (colors.Any(x =>
        {
            var color = x.Split(" ", StringSplitOptions.RemoveEmptyEntries);
            int amount = int.Parse(color[0]);
            return amount > cubeMax[color[1].Trim()];
        }))
        {
...
foreach(var line in input.Split(System.Environment.NewLine))
{
    ...
    var colors=sets
        ...
        {
            Color = z[1].Trim(),
            Amount = int.Parse(z[0])
        });
    var minimums=colors.GroupBy(x=>x.Color)
        .Select(y=>new
        {
            Color = y.Key,
            Amount = y.Max(z=>z.Amount)
        });
    var power=minimums.Aggregate(1,(acc, x) => acc * x.Amount);
    sumOfPowers += power;
}
